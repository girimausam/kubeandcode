...
Many teams **extract and load** raw data first (DMS, AppFlow, Firehose), then **transform in the warehouse or with Athena SQL** (ELT). Glue fits both models. Choose based on whether Spark complexity is worth it versus SQL-only transforms.

**Worked example (streams to lake):** [Glue, crawler, Kinesis, Firehose, and Lambda](./aws-etl-glue-kinesis-firehose-lambda.md) walks Northwind clickstream from Kinesis through Firehose, a transform Lambda, two crawlers, and a Glue job.
